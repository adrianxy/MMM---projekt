...
class PlotManager:
    ...
    def create_new_plot_area(self):
        ...
        def flexround(x, ndigits=3):
            if x == 0:
                return 0
            return round(x, -math.floor(math.log10(abs(x))) + ndigits)
        ...
        for k in range(self._configuration['ydiv_number']):
            ...
            self._canvas.create_line(20, PlotManager.height / 2 - y_offset, 40, PlotManager.height / 2 - y_offset)
            self._canvas.create_text(45, PlotManager.height / 2 - y_offset,
                                     text=flexround(k * self._configuration["yscale"]))
            self._canvas.create_text(45, PlotManager.height / 2 + y_offset,
                                     text=flexround(-k * self._configuration["yscale"]))
        ...
        for k in range(self._configuration['xdiv_number']):
            ...
            self._canvas.create_text(30 + x_offset, PlotManager.height / 2 + 15,
                                     text=flexround(k * self._configuration["xscale"]))
